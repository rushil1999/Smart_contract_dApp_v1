...
contract ProjectPool is Events {
    ...
    function addNewProject( 
        ...
        ProjectContract pc = new ProjectContract(_address, name, description, link, unit, valueRequired );  //New Party creates 
        projects.push(pc); //Pushed to public data
        totalEntries = totalEntries+1;
        emit NewProjectRegistered(_address, address(pc), totalEntries, name, description, link, unit, valueRequired);
    ...
    function getProjectDetails(uint256 index) external view returns(
        string memory name, 
        string memory description, 
        string memory link,
        string memory unit,
    ...
    function getProjectBalance(uint256 index) external view returns(uint256 projectBalance){
