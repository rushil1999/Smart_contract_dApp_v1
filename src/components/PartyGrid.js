...
const PartyGrid = props => {
  ...
  const registerParty = async () => {
    ...
    const newContractPartyAddress = await contractState.methods.addNewParty(userAccount[0], name,cause, link).call({from: userAccount[0]});
    ...
    <React.Fragment>
      ...
      <React.Fragment>
       {partyCount> 0 && (
         parties.map((e, index)=>{
          return(
            <Party index={index} userAccount={account}/>
          )
         })
       )} 
      </React.Fragment>
